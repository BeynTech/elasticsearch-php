...
BeynElasticsearch-php only has a three requirements that you need to worry about:
...
====
The default HTTP handlers that ship with BeynElasticsearch-php require the PHP libcurl extension, but it is not technically
...
You need to match your version of BeynElasticsearch to the appropriate version of this library.

The master branch will always track BeynElasticsearch master, but it is not recommended to use `dev-master` in your production code.
...
$client = BeynElasticsearch\ClientBuilder::create()->build();
--------------------------
+
...
a full copy of BeynElasticsearch so that tests can be run against the REST specifications.  This
