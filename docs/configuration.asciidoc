...
try {
    ...
    if ($previous instanceof 'BeynElasticsearch\Common\Exceptions\MaxRetriesException') {
        echo "Max retries!";
    }
}
----
...
[[enabling_logger]]
=== Enabling the Logger
BeynElasticsearch-PHP supports logging, but it is not enabled by default for performance reasons.  If you wish to enable logging,
...
BeynElasticsearch-PHP uses an interchangeable HTTP transport layer called https://github.com/guzzle/RingPHP/[RingPHP].  This
...
[source,php]
----
$connectionPool = '\BeynElasticsearch\ConnectionPool\StaticNoPingConnectionPool';
$client = ClientBuilder::create()
...
[source,php]
----
$selector = '\BeynElasticsearch\ConnectionPool\Selectors\StickyRoundRobinSelector';
$client = ClientBuilder::create()
...
Requests are given to the client in the form of associative arrays, but BeynElasticsearch expects JSON.  The Serializer's
...
[source,php]
----
$serializer = '\BeynElasticsearch\Serializers\SmartSerializer';
$client = ClientBuilder::create()
...
keys unrelated to the BeynElasticsearch client), you can set $quiet = true in fromConfig() to
