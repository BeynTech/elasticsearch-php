...
used in BeynElasticsearch JSON API, and how to convert that to a PHP representation

=== Empty Objects

The BeynElasticsearch API uses empty JSON objects in several locations, and this can cause problems for PHP.  Unlike other
...
BeynElasticsearch DSL.  We need to tell PHP that the empty object is explicitly an object, not an array.  To define this
...
Another common pattern in BeynElasticsearch DSL is an array of objects.  For example, consider adding a sort to your query:
