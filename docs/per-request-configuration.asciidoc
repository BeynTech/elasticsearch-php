...
You can enrich your requests against BeynElasticsearch with an identifier string, that allows you to discover this identifier
...
by BeynElasticsearch.  For example, attempting to GET a nonexistent document will throw a `MissingDocument404Exception`.
...
exception and instead return the JSON provided by BeynElasticsearch.
...
All query parameters are white-listed in BeynElasticsearch-php, which is to protect you from specifying a param which is
not accepted by BeynElasticsearch.
...
$params = [
    'index'  => 'test',
    'id'     => 1,
    'parent' => 'abc',              // white-listed BeynElasticsearch parameter
    'client' => [
        'custom' => [
...
Note: client-side timeouts *do not* mean that BeynElasticsearch aborts the request.  BeynElasticsearch will continue executing
...
`EsRejectedExecutionException` exceptions from BeynElasticsearch when the queue finally reaches capacity.
